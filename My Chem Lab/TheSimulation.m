...
{//calculate the reaction results, and determine whether the reaction was successful
    ...
    _Costs = ((initTemperature/1050 - 0.012)*initSimLength + initMethanol/5)/initOil;
    if(_Costs<0)_Costs=0.01;
...
- (NSDictionary*)getTheResult
{
    // The convout is converted here to a full decimal number
      return @{@"TGout": [NSNumber numberWithFloat:_TGout], @"DGout": [NSNumber numberWithFloat:_DGout],
                        @"MGout": [NSNumber numberWithFloat:_MGout], @"Eout": [NSNumber numberWithFloat:_Eout],
                        @"Convout": [NSNumber numberWithFloat:_Convout * 100], @"Cost":
               [NSNumber numberWithFloat:_Costs], @"mode": [NSNumber numberWithFloat:mode],
...
-(bool)loop
{
    ...
    if (timecount<(timetoreact+timetosettle)){
        ...
        if(lastTimeTargetAdded == 0 || now - lastTimeTargetAdded >= 0 ) {
            ...
            _Mout = cx[5];
            _Eout = cx[4];
            _Convout = cx[4]/(cx[4]+cx[1]+cx[2]+cx[3]);
            
            NSLog(@"The triglycerides: %f", _TGout);
            NSLog(@"The diglycerides: %f", _DGout);
            NSLog(@"The monoglycerides: %f", _MGout);
            NSLog(@"The biodiesl: %f", _Eout);
            NSLog(@"The conversion rate: %f", _Convout);
            NSLog(@"END LOOP");
        }
    } else
    {
